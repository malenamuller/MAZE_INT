...
#define BM_SCALE	2000
//
//int main(void) {
//	dpoint_t p[N_POINTS];
//
//	pointsOnPolygon(L_SARAH*BM_SCALE, N_POINTS, 8, 0, 0, p);
//
//	double max = 0;
//
//	for (unsigned int i = 0; i < N_POINTS; i++) {
//		max = std::max(max, p[i].x);
//		max = std::max(max, p[i].y);
//	}
//
//	al_init();
...
//	al_init_image_addon();
//
//	ALLEGRO_BITMAP * bm = al_create_bitmap(max, max);
//	al_set_target_bitmap(bm);
//
//	al_clear_to_color(al_map_rgb(255,255,255));
//	for (unsigned int i = 0; i < N_POINTS-1; i++) {
//		al_draw_line(p[i].x, p[i].y, p[i + 1].x, p[i + 1].y, al_map_rgb(0, 0, 0), .1*L_SARAH*BM_SCALE);
//	}
//	al_draw_line(p[0].x, p[0].y, p[N_POINTS-1].x, p[N_POINTS-1].y, al_map_rgb(0, 0, 0), .1*L_SARAH*BM_SCALE);
//
//
//	ALLEGRO_DISPLAY * d = al_create_display(1000, 700);
...
robot_t loadSarah(dpoint_t p0)
{
	...
	dpoint_t vertex[8];
	pointsOnPolygon(L_SARAH, 8, 8, 0, 0, vertex);
