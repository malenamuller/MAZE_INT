...
Dispatcher::Dispatcher(Graphic& g, rInfo_t * r) : g(g)
{
	...
	for (unsigned int i = 0; i < 2; i++) {
		r[0].r.sensorArray[i].positionOnRobot.x = S_getSensorXPos(i);
		r[0].r.sensorArray[i].positionOnRobot.y = S_getSensorYPos(i);
		r[0].r.sensorArray[i].angle = S_getSensorAngle(i);
	}
	W_setRobotConfiguration(&(r[0].r));
	I_Init(manual ? MANUAL : AUTO1);
	g.newRobot(r[0].img, r[0].size);
	g.drawMode(manual ? "manual" : "auto");
}
